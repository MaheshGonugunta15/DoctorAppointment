...
conn = pymysql.connect(host="localhost", user="root", password="root", db="DoctorAppointmentBooking")
cursor = conn.cursor()
...
def sendHospitalRequest():
    ...
    hospitals = cursor.fetchall()
    if len(hospitals) == 0:
        return render_template("msg.html", msg='Hospitals Not Available', color='text-primary')
...
def viewAvailableTimings():
    ...
    return render_template("viewAvailableTimings.html", get_validation=get_validation,doctorRequests=doctorRequests2, doctor_id=doctor_id, booking_date2=booking_date2, booking_date=booking_date, day=day, isSlotBooked=isSlotBooked, getHospital_by_doctor_request_id=getHospital_by_doctor_request_id)
...
def bookAppointment1():
    ...
    user = cursor.fetchall()
    email = user[0][2]
    send_email("Doctor Appointment", "Your  Appointment Booked on '"+booking_date+"'  "+str(fromTime) + " - " + str(toTime)+" \n Clik Here To Cancel  http://127.0.0.1:5000/cancel_appointment?booking_id="+str(booking_id)+" \n or Reschedule Appointment http://127.0.0.1:5000/reschedule_appointment?booking_id="+str(booking_id)+"", email)
...
def cancel_appointment():
    ...
    a = cursor.execute("select * from Bookings where status = 'Doctor Appointment Booked'  and booking_id='"+str(booking_id)+"'")
    if a > 0:
        a = cursor.execute("update Bookings set status = 'Appointment Cancelled' where booking_id='"+str(booking_id)+"'")
        conn.commit()
        return render_template("msg.html", msg='Appointment Cancelled',color='text-danger')
    else:
        return render_template("msg.html", msg='Already Appointment Cancelled', color='text-danger')


@app.route("/reschedule_appointment")
def reschedule_appointment():
    booking_id = request.args.get("booking_id")
    cursor.execute("select * from doctor where doctor_id in (select doctor_id from doctorrequests where doctorRequest_id in (select doctorRequest_id from slots where slot_id in (select slot_id from Bookings where  booking_id='"+str(booking_id) +"')))")
    details = cursor.fetchall()
    doctor_id = details[0][0]

    a = cursor.execute("select * from Bookings where status = 'Doctor Appointment Booked'  and booking_id='"+str(booking_id)+"'")
    if a>0:
        cursor.execute("update Bookings set status = 'Appointment Cancelled' where booking_id='"+str(booking_id) +"'")
        conn.commit()
        if 'User' not in session['role']:
            return uLogin()
        return redirect("viewAvailableTimings?doctor_id="+str(doctor_id))
    else:
        return render_template("msg.html", msg='Already Appointment Rescheduled', color='text-danger')
...
def get_validation(booking_date):
    count = cursor.execute("select * from Bookings where booking_date='"+str(booking_date)+"' and status!='Appointment Cancelled' and status!='Booking Cancelled By User' and user_id='"+str(session['user_id'])+"'")
    return count
